...
JANELAS = [720] if (args.test or args.volpi) else [15, 30, 60, 360, 720, 1440]
...
def rodar_compressor(algo, serie, target_cr):
    if algo == "wavelet":
        cr_param = 100 / (100 - target_cr)
        c = WaveletCompressor(cr=cr_param)
        c.compress(serie)
    elif algo == "dct":
        cr_param = 100 / (100 - target_cr)
        c = DCTCompressor(cr=cr_param)
        c.compress(serie)
    elif algo == "arcsdt":
        ...
        for target_cr in TARGET_CRS:
            for janela in JANELAS:
                for algo, label in algos_ativos:
                    ...
                    # divide série em janelas
                    janelas = [serie[i:i+janela] for i in range(0, len(serie), janela)]
                    # descarta última janela se incompleta
                    janelas = [j for j in janelas if len(j) == janela]

                    if args.volpi:
                        print(f"  [{data}]  alvo: {target_cr}%  janelas: {len(janelas)}")
                        print(f"  {'Janela':<8} {'CR':>8}")
                        print(f"  {'─'*18}")
                    ...
                    for idx, j in enumerate(janelas):
                        ...
                        if args.volpi:
                            print(f"  {idx+1:<8} {c.compression_ratio:>7.2f}%")
...
print(f"\n  ✓ Concluído! {len(resultados)} experimentos salvos em: {caminho_csv}\n")