"""
process_energy_csv9.py
----------------------
Lê um arquivo separado por espaços, sem header.
Coluna 0: timestamp Unix em segundos (float).
Coluna 1: mains em W.
Dados a ~1 segundo → reamostra para 1 minuto (média).
Encontra a primeira meia-noite UTC e gera até 7 arquivos JSON com os dias completos.

Uso:
    python process_energy_csv9.py <caminho_do_arquivo> [--prefix PREFIXO]
"""
...
def load_and_prepare(csv_path: Path) -> pd.Series:
    """Lê o arquivo, usa coluna 1 como mains e reamostra para 1 minuto."""
    df = pd.read_csv(
        csv_path,
        sep=r"\s+",
        header=None,
        usecols=[0, 1],
        names=["timestamp", "mains"],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Processa arquivo separado por espaços (timestamp mains ...) e gera JSONs diários."
    )
