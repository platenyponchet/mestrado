"""
process_energy_csv2.py
----------------------
Lê um CSV sem header com timestamps em milissegundos e valores de mains,
reamostra para 1 minuto, encontra a primeira meia-noite e gera até 7
arquivos JSON com os dias completos.

Uso:
    python process_energy_csv2.py <caminho_do_csv> [--prefix PREFIXO]
"""
...
def load_and_prepare(csv_path: Path) -> pd.Series:
    """Lê o CSV, converte timestamp em ms, ordena e reamostra para 1 min."""
    df = pd.read_csv(csv_path, header=None, names=["unix_ms", "mains"])

    df["datetime"] = pd.to_datetime(df["unix_ms"], unit="ms", utc=True)
    df = df.drop(columns=["unix_ms"])

    df = df.sort_values("datetime").set_index("datetime")

    # Reamostra para 1 minuto usando a média dos ~2 valores de 30s
    # min_count=1 garante que minutos sem nenhum dado virem NaN (não 0)
...
def extract_complete_days(
    ...
    sem nenhum NaN (minuto sem dado = dia ignorado).
    """
    complete_days = []
    current = start
    ...
    while len(complete_days) < max_days:
        ...
        # Seleciona a janela do dia e reindexa para garantir todos os 1440 min
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Processa CSV de energia (sem header, ms) e gera JSONs diários.")
