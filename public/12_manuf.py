"""
process_energy_csv12.py
-----------------------
Lê um CSV com header (Timestamp, Temperature, Machine Speed, ..., Energy Consumption (kWh), ...).
Usa 'Energy Consumption (kWh)' como mains e converte kWh → Wh (*1000).
Dados já em resolução de 1 minuto.
Encontra a primeira meia-noite UTC e gera até 7 arquivos JSON com os dias completos.

Uso:
    python process_energy_csv12.py <caminho_do_csv> [--prefix PREFIXO]
"""
...
MAINS_COL = "Energy Consumption (kWh)"
DATETIME_COL = "Timestamp"


def load_and_prepare(csv_path: Path) -> pd.Series:
    """Lê o CSV, converte Energy Consumption de kWh para Wh e reamostra para 1 minuto."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Processa CSV industrial usando 'Energy Consumption (kWh)' como mains."
    )
    ...
    print(f"   Coluna mains: '{MAINS_COL}' (convertida kWh → Wh)")
