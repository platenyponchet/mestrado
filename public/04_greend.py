...
def pick_mains_column(df: pd.DataFrame, timestamp_col: str) -> str:
    """
    Retorna o nome da coluna com maior amplitude (max - min),
    ignorando a coluna de timestamp e colunas 100% NaN.
    """
    candidates = [c for c in df.columns if c != timestamp_col]
    numeric = df[candidates].apply(pd.to_numeric, errors="coerce")

    amplitudes = numeric.max() - numeric.min()
    amplitudes = amplitudes.dropna()

    if amplitudes.empty:
        raise ValueError("Nenhuma coluna numérica válida encontrada além do timestamp.")

    chosen = amplitudes.idxmax()
    print(f"\n📊 Amplitudes por coluna:")
    for col, amp in amplitudes.sort_values(ascending=False).items():
        marker = " ◄ selecionada" if col == chosen else ""
        print(f"   {col}: {amp:.4f} W{marker}")
