"""
process_energy_csv5.py
----------------------
Lê um CSV com header (timestamp, W, VAR, VA, f, VLN, PF, A).
Usa a coluna 'W' como mains, reamostra para 1 minuto (média),
encontra a primeira meia-noite e gera até 7 arquivos JSON com os dias completos.

Uso:
    python process_energy_csv5.py <caminho_do_csv> [--prefix PREFIXO]
"""
...
def load_and_prepare(csv_path: Path) -> pd.Series:
    """Lê o CSV, usa a coluna W como mains e reamostra para 1 minuto."""
    df = pd.read_csv(csv_path, na_values=["NULL", "null", "NaN", "nan", ""])

    if "timestamp" not in df.columns:
        raise ValueError("Coluna 'timestamp' não encontrada no arquivo.")
    if "W" not in df.columns:
        raise ValueError("Coluna 'W' não encontrada no arquivo.")
    ...
    series = df["W"].resample("1min").mean()
    return series
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Processa CSV com colunas elétricas (W, VAR, VA...) e gera JSONs diários."
    )
